...
def getClassScore(charSamp, Nway=10, frontierSize=5, nUpdates=10, plotON=False):
    ...
    model.conditional(i=idx_all[0:1], D=x_all[0:1], x=[x_samp] * (len(idx_all)))

    scores_sampmodel = [model.conditional(i=np.array(idx_all[0:1]), D=x_all[0:1], x=x_all[kk:kk+1]) for kk in range(1, len(x_all))]
    scores_sampmodel = torch.tensor(np.concatenate(a))
    ...
    print(scores)
    print(idx_all)
    return scores, scores_sampmodel, idx_all
...
for i, cc in enumerate(charall):
    cc = np.array(cc)
    scores, scores_sampmodel, idx = getClassScore(charSamp=cc, Nway=20, frontierSize=8, nUpdates=20, plotON=False)
